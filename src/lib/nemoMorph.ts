 ...
 *
 * scripts/generate-nemo-loader.mjs bakes the two frames (the numerals at the
 * 100% frame — tight tracking — and the character refit inside itself)
 * together with the type metrics they share. This module turns those metrics
 * back into live layout, mirroring the generator's formula exactly:
 *
 ...
 * so the counter stays dead centre while the tracking contracts. At 100% that
 * reproduces the generator's reference ink box, which is exactly what lets the
 * counter hand its ink to MorphSVG without a seam (asserted in
 * tests/unit/nemoLoader.test.ts).
 *
...
/** The counter's tracking range, in viewBox units. TRACK_FROM is the wide
 *  opening frame; TRACK_TO the tight frame the climb contracts into. */
...
/** Em tracking → viewBox units across the counter's climb. The endpoints are
 *  returned exactly (no lerp residue): at 100% the counter must sit on the
 *  baked reference frame to the last unit before MorphSVG takes the ink. */
export function trackingAt(progress: number): number {
  if (progress <= 0) return TRACK_FROM;
  if (progress >= 1) return TRACK_TO;
  return TRACK_FROM + (TRACK_TO - TRACK_FROM) * progress;
}
