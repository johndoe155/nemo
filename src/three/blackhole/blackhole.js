...
import { flatSimulationConfig } from './blackhole.config.js';
...
export class BlackHoleSimulation {
  constructor(scene) {
    this.scene = scene;
    // Parameters come directly from the dedicated config file
    // (blackhole.config.js) - the single source of truth.
    this.config = flatSimulationConfig;
    this.blackHoleMesh = null;
    this.initializeUniforms(this.config);
  }

  /**
   * Initialize all shader uniforms from the static configuration
   * (blackhole.config.js).
   */
  ...
  /**
   * Update uniform values from a config object.
   * Programmatic API for runtime tweaks; the initial values are loaded
   * from the static configuration file (blackhole.config.js).
   */
